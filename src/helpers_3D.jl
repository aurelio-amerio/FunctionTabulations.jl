function _load_file_3D(filepath, xmin, xmax, ymin, ymax, zmin, zmax, npoints_x, npoints_y, npoints_z, metadata, metadata_validation_fn)
    if isfile(filepath)
        data = load(filepath)
...
function _compute_matrix_and_save_3D(func, arg, x, y, z, x_scale, y_scale, z_scale, xmin, xmax, ymin, ymax, zmin, zmax, npoints_x, npoints_y, npoints_z, metadata, filepath, filename, compress)
...
function _get_interp_fn_3D(x, y, z, data_matrix, x_scale, y_scale, z_scale, f_scale, x_units, y_units, z_units, f_units, interpolation_type, extrapolation_bc)
    if f_scale == :log10
    ...
    return wrap_function_3D_add_units(func_interp, x_units, y_units, z_units, f_units)
end